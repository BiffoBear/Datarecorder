...
'''
        returned_result = addnode.layout_existing_things(thing_name='sensor', existing_things=[1, 5])
        self.assertEqual(expected_result, returned_result)

    def test_layout_existing_things_handles_empty_lists(self):
        expected_result = 'No existing nodes in database'
        returned_result = addnode.layout_existing_things('node', [])
        self.assertEqual(expected_result, returned_result)
        expected_result = 'No existing sensors in database'
        returned_result = addnode.layout_existing_things('sensor', [])
        self.assertEqual(expected_result, returned_result)
...
'''
        returned_result = addnode.layout_thing_details(thing_name='sensor', thing_id=1, thing_data=data)
        self.assertEqual(expected_result, returned_result)
        data = {'Node_ID': 2,
                'Name': 'Node name',
                'Location': 'A location',
...
'''
        returned_result = addnode.layout_thing_details(thing_name='node', thing_id=2, thing_data=data)
        self.assertEqual(expected_result, returned_result)
