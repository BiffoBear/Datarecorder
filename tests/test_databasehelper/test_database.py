...
class ConfirmDatabaseSetup(TestCase):
    def setUp(self):
        self.required_tables_and_columns = {
            "Sensor Readings": [
                "ID",
                "Timestamp_UTC",
                "Sensor_ID",
    ...
    def test_database_tables_exist(self):
        self.assertCountEqual(
            self.inspector.get_table_names(), self.required_tables_and_columns.keys()
        )
...
class TestWriteSensorDataToDataBase(TestCase):
    ...
    def test_write_sensor_data_to_database(self):
        ...
        test_data = {
            "timestamp": test_time,
            "sensor_readings": [(0x01, 1.2345), (0x02, 2.3456)],
        }
        ...
        expected_result = [
            [test_time, x[0], x[1]] for x in test_data["sensor_readings"]
        ]
...
class TestAddNodeToDataBase(TestCase):
    ...
    def test_check_id_and_name_is_valid_raises_typeerror_if_id_not_int(self):
        ...
        test_data = (
            {"id": "a", "name": "A Name", "type": "node", "error": node_error_message},
            {
                "id": "a",
                "name": "A Name",
                "type": "sensor",
                "error": sensor_error_message,
            },
            {
                "id": None,
                "name": "A Name",
        ...
        for data in test_data:
            with self.assertRaises(TypeError) as dm:
                database._check_id_and_name_are_valid(
                    id_to_check=data["id"],
    ...
    def test_check_id_and_name_is_valid_raises_valueerror_if_id_not_in_range(self):
        node_error_message = (
            "Record not created, node ID must be in range 0 - 254 (0x00 - 0xfe)"
        )
        sensor_error_message = (
            "Record not created, sensor ID must be in range 0 - 254 (0x00 - 0xfe)"
        )
        test_data = (
            {"id": 0xFF, "name": "A Name", "type": "node", "error": node_error_message},
            {
                "id": 0xFF,
                "name": "A Name",
                "type": "sensor",
    ...
    def test_check_id_and_name_is_valid_raises_typeerror_if_name_not_string_starting_with_a_letter(
        self,
    ):
        node_error_message = (
            "Record not created, node name must be a string beginning with a letter"
        )
        sensor_error_message = (
            "Record not created, sensor name must be a string beginning with a letter"
        )
        test_data = (
            {"id": 0x05, "name": 2, "type": "node", "error": node_error_message},
    ...
    def test_add_node_adds_unique_valid_data_that_it_is_called_with(self):
        test_data = [
            [0x01, "Test node name", "Dummy location"],
    ...
    def test_add_node_raises_valueerror_for_repeat_id(self):
        ...
        with self.assertRaises(ValueError) as dm:
            [
                database.add_node(node_id=x[0], name=x[1], location=x[2])
                for x in test_data
            ]
        database_records = get_all_nodes()
        ...
        self.assertEqual(database_records, expected_result)
        self.assertIn(
            "Record not created, node ID and name must be unique", dm.exception.args
        )
...
class TestAddSensorToDatabase(TestCase):
    ...
    def test_add_sensor_adds_unique_data_that_it_is_called_with(
        self, mock_node_id_exists
    ):
        mock_node_id_exists.return_value = True
        test_data = [
            [0x01, 0x01, "Dummy Sensor", "Mass"],
            [0x02, 0x01, "Another Sensor", "Mass"],
        ]
        [
            database.add_sensor(sensor_id=x[0], node_id=x[1], name=x[2], quantity=x[3])
            for x in test_data
        ]
    ...
    def test_add_sensor_raises_valueerror_for_repeat_id(self, mock_node_id_exists):
        ...
        with self.assertRaises(ValueError) as dm:
            [
                database.add_sensor(
                    sensor_id=x[0], node_id=x[1], name=x[2], quantity=x[3]
                )
                for x in test_data
            ]
        database_records = get_all_sensors()
        ...
        self.assertEqual(database_records, expected_result)
        self.assertIn(
            "Record not created, Sensor ID and name must be unique", dm.exception.args
        )
    ...
    def test_add_sensor_raises_valueerror_if_node_id_does_not_exist(
        self, mock_node_id_exists
    ):
        ...
        self.assertIn(
            "Record not created -- node with id 1 (0x01) must already exist in the database",
            dm.exception.args,
        )

    def test_add_sensor_only_raises_value_error_when_quantity_not_in_dict(
        self, mock_node_id_exists
    ):
        mock_node_id_exists.return_value = True
        ...
        self.assertIn(
            "Sensor not created -- unknown sensor data quantity supplied",
            dm.exception.args,
        )
...
class TestReadFromDatabaseFunctions(TestCase):
    ...
    @patch("database.database._node_id_exists")
    def test_sensor_id_exists_returns_true_if_sensor_exists_false_if_not(
        self, mock_node_id_exists
    ):
        mock_node_id_exists.return_value = True
    ...
    @patch("database.database._node_id_exists")
    def test_get_all_sensor_ids_returns_a_generator_with_all_sensor_ids(
        self, mock_node_id_exists
    ):
        mock_node_id_exists.return_value = True
        test_data = [
            [0x01, 0x01, "Sensor 1", "Mass"],
    ...
    @patch("database.database._node_id_exists")
    def test_get_all_ids_returns_a_generator_with_correct_values(
        self, mock_node_id_exists
    ):
        mock_node_id_exists.return_value = True
        test_data_nodes = [
        ...
        [
            database.add_sensor(sensor_id=x[0], node_id=x[1], name=x[2], quantity=x[3])
            for x in test_data_sensors
        ]
        returned_result = database._get_all_ids(table="node")
        self.assertListEqual(
            [x[0] for x in test_data_nodes], [y for y in returned_result]
        )
        returned_result = database._get_all_ids(table="sensor")
        self.assertListEqual(
            [x[0] for x in test_data_sensors], [y for y in returned_result]
        )
    ...
    def test_get_sensors_for_a_node(self, mock_node_id_exists):
        ...
        self.assertListEqual(
            [x[0] for x in test_data_sensors[0:3]], [y for y in returned_result]
        )
    ...
    def test_get_sensors_for_a_node_raises_noresultfound_exception_if_node_does_not_exist(
        self,
    ):
    ...
    def test_get_node_or_sensor_returns_all_data_for_the_query(self):
        ...
        self.assertListEqual(
            ...
            [
                returned_result.ID,
                returned_result.Node_ID,
                returned_result.Name,
                returned_result.Quantity,
            ],
        )
    ...
    def test_get_node_or_sensor_raises_typeerror_if_not_called_with_int(self):
        ...
        self.assertIn(
            "sensor must be an integer (not <class 'str'>)", dm.exception.args
        )

    def test_get_node_or_sensor_raises_noresultfound_exception_if_record_does_not_exist(
        self,
    ):
        with self.assertRaises(NoResultFound) as dm:
    ...
    def test_get_node_data_returns_a_dict(self):
        ...
        [database.add_node(node_id=x[0], name=x[1], location=x[2]) for x in test_data]
        expected_result = {
            "Node_ID": test_data[0][0],
            "Name": test_data[0][1],
            "Location": test_data[0][2],
        }
    ...
    def test_get_sensor_data_returns_a_dict(self, mock_node_id_exists):
        ...
        expected_result = {
            "Sensor_ID": test_data[0][0],
            "Node_ID": test_data[0][1],
            "Name": test_data[0][2],
            "Quantity": test_data[0][3],
        }
