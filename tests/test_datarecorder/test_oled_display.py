...
class TestDisplayHardwareSetup:
    ...
    def test_initialize_oled_returns_none_if_setup_fails(self, mocker):
        ...
        assert _oleddisplay.initialize_oled('dummy i2c', reset_pin='dummy reset') is None

    def test_setup_hardware_returns_result_of_initialize_oled(self, mocker):
        _ = mocker.patch.object(datarecorder._oleddisplay, "initialize_i2c", autospec=True)
        mock_initialize_oled = mocker.patch.object(datarecorder._oleddisplay, "initialize_oled", autospec=True)
...
class TestTextImageWriting:
    ...
    def test_setup_display_dict_returns_dictionary_object(self, mocker):
        mock_setup_hardware_oled = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        ...
        assert list(returned_result.keys()) == ['oled', 'image', 'draw', 'font', 'lines']
    ...
    def test_clear_display_returns_display_and_calls_pil_image_draw_with_correct_values(self, mocker):
    ...
    def test_write_text_to_display_returns_display_and_is_called_with_correct_args(self):
        ...
        coord = (1, 1)
        assert _oleddisplay.write_text_to_display(display=mock_display, coords=coord, text='Hello World') == mock_display
...
class TestTextDeliveryAndLayout:
    ...
    def test_add_screen_line_maintains_max_5_item_fifo_queue(self, mocker):
        _1 = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        ...
        _4 = mocker.patch.object(datarecorder._oleddisplay, "write_text_to_display", autospec=True)
    ...
    def test_lines_are_drawn_at_correct_coordinates(self, mocker):
        _1 = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        mock_clear_display = mocker.patch.object(datarecorder._oleddisplay, "clear_display", autospec=True)
        _3 = mocker.patch.object(datarecorder._oleddisplay, "show_display", autospec=True)
        mock_write_text_to_display = mocker.patch.object(datarecorder._oleddisplay, "write_text_to_display", autospec=True)
    ...
    def test_draw_lines_calls_clear_display(self, mocker):
        _1 = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        mock_clear_display = mocker.patch.object(datarecorder._oleddisplay, "clear_display", autospec=True)
        mock_show_display = mocker.patch.object(datarecorder._oleddisplay, "show_display", autospec=True)
        _4 = mocker.patch.object(datarecorder._oleddisplay, "write_text_to_display", autospec=True)
...
class TestDataFlow:
    ...
    def test_display_message_from_queue_calls_draw_lines_if_oled_not_None(self, mocker):
        mock_draw_lines = mocker.patch.object(datarecorder._oleddisplay, "draw_lines", autospec=True)
        mock_message_queue = mocker.patch.object(datarecorder._oleddisplay, "message_queue", autospec=True)
    ...
    def test_display_message_from_queue_does_not_call_draw_lines_if_oled_None(self, mocker):
        mock_draw_lines = mocker.patch.object(datarecorder._oleddisplay, "draw_lines", autospec=True)
        mock_message_queue = mocker.patch.object(datarecorder._oleddisplay, "message_queue", autospec=True)
        mock_message_queue.get.return_value = 'dummy text'
    ...
    def test_display_message_from_queue_returns_lines_and_display(self, mocker,):
        mock_draw_lines = mocker.patch.object(datarecorder._oleddisplay, "draw_lines", autospec=True)
    ...
    def test_display_message_from_queue_raises_exception_and_calls_task_done(self, mocker):
        mock_add_screen_line = mocker.patch.object(datarecorder._oleddisplay, "add_screen_line", autospec=True)
        mock_message_queue = mocker.patch.object(datarecorder._oleddisplay, "message_queue", autospec=True)
    ...
    def test_display_is_passed_up_and_down_correctly(self, mocker):
        mock_show_display = mocker.patch.object(datarecorder._oleddisplay, "show_display", autospec=True)
        mock_setup_hardware_oled = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        mock_write_text_to_display = mocker.patch.object(datarecorder._oleddisplay, "show_display", autospec=True)
...
class TestInitAndThreadingCalls:
    ...
    def test_setup_called_and_thread_called_and_started_as_daemon(self, mocker):
        ...
        mock_loop_read_message_queue = mocker.patch.object(datarecorder._oleddisplay, "loop_read_message_queue", autospec=True)
        mock_setup_display_dict = mocker.patch.object(datarecorder._oleddisplay, "setup_display_dict", autospec=True)
        mock_setup_hardware_oled = mocker.patch.object(datarecorder._oleddisplay, "setup_hardware_oled", autospec=True)
        mock_setup_hardware_oled.return_value = 'dummy display'
