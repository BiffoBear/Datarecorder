...
class TestMainLoggingCalls:
    ...
    def test_initialize_processing_thread(self, mocker):
        _ = mocker.patch.object(
            datarecorder._dataprocessing, "init_data_processing_thread", autospec=True
        )
        with self.assertLogs(level="DEBUG") as lm:
    ...
    def test_start_up(self, mocker):
        _1 = mocker.patch.object(
            datarecorder.main, "initialize_gpio_interrupt", autospec=True
        )
        _2 = mocker.patch.object(
            datarecorder.main, "initialize_processing_thread", autospec=True
        )
        _3 = mocker.patch.object(
            datarecorder.main, "initialize_database", autospec=True
        )
        _4 = mocker.patch.object(datarecorder.main, "initialize_logging", autospec=True)
        _5 = mocker.patch.object(
            datarecorder._oleddisplay, "init_display_thread", autospec=True
        )
        mock_rfm = mocker.patch.object(
            datarecorder.main, "initialize_rfm69", autospec=True
        )
    ...
    def test_shutdown(self, mocker):
        ...
        _2 = mocker.patch.object(
            datarecorder._dataprocessing, "radio_q.join", autospec=True
        )
...
class TestDataProcessing:
    ...
    def test_process_radio_data(self, mocker):
        mock_packet_missing_or_duplicate = mocker.patch.object(
            datarecorder._dataprocessing, "packet_missing_or_duplicate", autospec=True
        )
        mock_expand_radio_data_into_dict = mocker.patch.object(
            datarecorder._dataprocessing, "expand_radio_data_into_dict", autospec=True
        )
        mock_unpack_data_packet = mocker.patch.object(
            datarecorder._dataprocessing, "unpack_data_packet", autospec=True
        )
        mock_radio_q = mocker.patch.object(
            datarecorder._dataprocessing, "radio_q", autospec=True
        )
...
class TestOledDisplay:
    ...
    def test_initialize_oled(self, mocker):
        ...
        with self.assertLogs(level="ERROR") as cm:
            _oleddisplay.initialize_oled(None, None)
        self.assertIn(
            "OLED display failed to initialize. Check that wiring is correct",
            cm.output[0],
        )
        with self.assertLogs(level="ERROR") as cm:
            _oleddisplay.initialize_oled(None, None)
        self.assertIn(
            "OLED display failed to initialize. No I2C bus found", cm.output[0]
        )
    ...
    def test_write_text_to_display(self, mocker):
        ...
        with self.assertLogs(level="DEBUG") as cm:
            _oleddisplay.write_text_to_display(
                display=dummy_display, coords=(0, 0), text=""
            )
