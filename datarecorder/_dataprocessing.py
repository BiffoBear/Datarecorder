...
def packet_missing_or_duplicate(node_data):
    ...
    try:
        ...
        logger.info("Rx from node 0x%2.2x, packet serial 0x%4.4x", node_id,
                                                                   new_packet_serial_number)
        _oleddisplay.write_message_to_queue(f"First data node 0x{node_id:02x}")
        _oleddisplay.write_message_to_queue(f"Rx {node_id:02x} sn 0x{new_packet_serial_number:04x}")
        last_packet_info[node_id] = {'pkt_serial': new_packet_serial_number,
                                     'timestamp': datetime.utcnow()
                                     }
        return False
    if new_packet_serial_number != old_packet_serial_number:
        if new_packet_serial_number != radiohelper.Increment_with_wrap(old_packet_serial_number):
        ...
        logger.info("Rx from node 0x%2.2x, packet serial 0x%4.4x", node_id,
                                                                   new_packet_serial_number)
        _oleddisplay.write_message_to_queue(f'Rx 0x{node_id:02x} '
                                            'sn 0x{new_packet_serial_number:04x}')
        return False
    return True
